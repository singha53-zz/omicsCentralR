#' Update omics central packages
#'
#' This will check to see if all omics central packages (and optionally, their
#' dependencies) are up-to-date, and will install after an interactive
#' confirmation.
#'
#' @inheritParams omicscentral_deps
#' @export
#' @examples
#' \dontrun{
#' omicscentral_update()
#' }
omicscentral_update <- function(recursive = FALSE, repos = getOption("repos")) {

  deps <- omicscentral_deps(recursive, repos)
  behind <- dplyr::filter(deps, behind)

  if (nrow(behind) == 0) {
    cli::cat_line("All omicscentral packages up-to-date")
    return(invisible())
  }
...
#' List all Omics Central dependencies
#'
#' @param recursive If \code{TRUE}, will also list all dependencies of
#'  Omics Central packages.
#' @param repos The repositories to use to check for updates.
#'   Defaults to \code{getOptions("repos")}.
#' @export
omicscentral_deps <- function(recursive = FALSE, repos = getOption("repos")) {
  pkgs <- utils::available.packages(repos = repos)
  deps <- tools::package_dependencies("omicscentral", pkgs, recursive = recursive)
